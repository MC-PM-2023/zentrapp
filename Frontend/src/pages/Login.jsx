  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (role?.trim().toLowerCase() === 'admin') {
        ...
        navigate('/home'); // regular user goes to their assigned apps page
      }
    ...
    <div className="login-container">
      <div className="login-left d-flex flex-column justify-content-center p-5 align-items-center">
        ...
        <form onSubmit={handleLogin} style={{ maxWidth: '400px', width: '100%' }}>
          ...
          <p className="mt-2 text-light small text-center">
            Don’t have an account?{' '}
            <NavLink to="/signup" className="text-decoration-none text-white">Sign up</NavLink>
          </p>
        </form>
      </div>
      <div className="login-right">
        <img src={login} className='image-placeholder' alt="Login" />
      </div>
      <ToastContainer />
    </div>
  );
